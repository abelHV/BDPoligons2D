...
{
    public class ClHexagonBD : ClPoligonBD
    {
        public override string tipusPoligon => "Hexàgon";
        ...
        public override Boolean getDadesPoligon(ClBdMySQL xbd, Int64 xid)
        {
            ...
            idPoligon = xid;
            String xsql = $"SELECT * FROM tbPoligon p INNER JOIN tbHexagon q ON p.IdPoligon=q.IdPoligon WHERE p.IdPoligon={idPoligon}";
        ...
        public override Boolean addPoligonBD(ClBdMySQL xbd)
        {
            ...
            if (idPoligon > 0)
            {
                String xsql = $"INSERT INTO tbHexagon (IdPoligon, `Radi`) VALUES ({idPoligon}, {Radi})";
