...
{
    public class ClHeptagonBD : ClPoligonBD
    {
        public override string tipusPoligon => "Heptàgon";
        ...
        public override Boolean getDadesPoligon(ClBdMySQL xbd, Int64 xid)
        {
            ...
            idPoligon = xid;
            String xsql = $"SELECT * FROM tbPoligon p INNER JOIN tbHeptagon q ON p.IdPoligon=q.IdPoligon WHERE p.IdPoligon={idPoligon}";
        ...
        public override Boolean addPoligonBD(ClBdMySQL xbd)
        {
            ...
            if (idPoligon > 0)
            {
                String xsql = $"INSERT INTO tbHeptagon (IdPoligon, `Radi`) VALUES ({idPoligon}, {Radi})";
