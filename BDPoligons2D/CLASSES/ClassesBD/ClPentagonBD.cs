...
{
    public class ClPentagonBD : ClPoligonBD
    {
        public override string tipusPoligon => "Pentàgon";
        ...
        public override Boolean getDadesPoligon(ClBdMySQL xbd, Int64 xid)
        {
            ...
            idPoligon = xid;
            String xsql = $"SELECT * FROM tbPoligon p INNER JOIN tbPentagon q ON p.IdPoligon=q.IdPoligon WHERE p.IdPoligon={idPoligon}";
        ...
        public override Boolean addPoligonBD(ClBdMySQL xbd)
        {
            ...
            if (idPoligon > 0)
            {
                String xsql = $"INSERT INTO tbPentagon (IdPoligon, `Radi`) VALUES ({idPoligon}, {Radi})";
