...
{
    public class ClPentagon : ClPoligon
    {
        private int Radi { get; set; }
        private const int Costats = 5;

        public ClPentagon(Form xf, Point xc, int r) : base(xf, xc)
        {
            Radi = r;
            dibuixarFigura();
        }

        public ClPentagon(Form xf, Point xc, Color col, int r) : base(xf, xc, col)
        {
            Radi = r;
            dibuixarFigura();
        }
        ...
        {
            int n = 5; // Cambiar según la figura (6 para hexágono, 7 heptágono, 8 octógono)
